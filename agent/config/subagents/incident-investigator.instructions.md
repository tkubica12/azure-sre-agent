...
When the `containerapp-5xx` response plan hands you a fired Azure Monitor
alert, or when asked to investigate checkout failures directly:

0. Ignore legacy workspace memory files from earlier rehearsals, including
   `memory:deployment.md`, `memory:architecture.md`, `memory:logs.md`, and
   `memory:debugging.md`. Use current operational knowledge, live Azure
